    ...
    public static void main(String[] args) {
        try {
            ...
            while (testcases-- > 0) {
                ...
                if(nums[siz-1] < nums[siz-2]) out.println(-1); // if the last elemnt is smaller we can't reduce that so ans = -1;
                else {
                    if(nums[siz-1] >= 0) { // if the last element is negative, then the array should be sorted
                        out.println(siz-2);
                        for(int i = 1; i < siz-1; i++) {
                            out.println(i+" "+(siz-1)+" "+siz);
                        }
                    } else {
                        boolean poss = true;
                        for(int i = 1; i < siz; i++) poss &= (nums[i] >= nums[i-1]); // checking if the array is sorted
                        out.println(poss?"0":"-1");
                    }
                }
