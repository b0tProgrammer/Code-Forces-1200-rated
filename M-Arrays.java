...
//            Comparator<Pair> com = (i, j) -> (int) ((i.val - j.val));
            ...
            while (testcases-- > 0) {
                /*
                    M-arrays in this, we have to output the minimum number of arrays that are to be formed in-order to make the every adjacent element's in that array divisible by the value M
                    so, we can prove that in the following way!
                    for example m = 8
                    1 1 1 5 2 4 4 8 6 7
                    1 1 1 5 2 4 4 0 6 7 -> moded array
                    map {
                        1 - 3
                        5 - 1
                        2 - 1
                ...
                for(Map.Entry<Integer,Integer> ma : map.entrySet()) {
                    if (ma.getKey() == 0) ans++;
                    else if (2 * ma.getKey() == m) ans++;
                    else if (2 * ma.getKey() < m || !map.containsKey(m - ma.getKey())) {
                        int x = ma.getKey(); // 1
                        int y = ma.getValue(); // 3
                        int z = map.getOrDefault(m - x, 0); // checking for 7 frequency -> 1
                        ans += (1 + Math.max(0, Math.abs(y - z) - 1));
                    }
                }
                out.println(ans);
            }
